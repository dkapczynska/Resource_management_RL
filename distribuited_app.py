...
class DistributedApp(Env):
    def __init__(self,
        ...
        self.observation_space = Box(low=np.array([-500]), high=np.array([500]))
        self.sim_length = 60
    ...
    def _calculate_cost(self):
        ...
        self.cost = np.round_((vm1_cost + vm2_cost + vm3_cost)/3, 2)  # np.sqrt
    ...
    def _calculate_utility_function(self):
        if self.qos >= self.sla:
            ...
            penalty = 10
        self.state = ((self.state + self.cost)/2) + penalty

    def _calculate_quality_of_service(self):
        if self.request_completed == 0:
            return 0
        else:
            self.qos = np.round_(self.request_completed / (61 - self.sim_length), 4)
    ...
    def step(self, action):
        ...
        requests_possible_to_complete = np.floor(self.computing_power / vm_total_count)
